...
| 8 | Vault feature (always-locked notes) + 2FA (Business+) | 2 | shipped | `Vault.tsx`, `TFA.tsx`, `Settings/Security.tsx`, `crypto.ts` (`deriveVaultKey`/`encryptVaultBody`/`decryptVaultBody`), `meo.notes.is_vault`, `meo.tfa_secrets`, `tfa-enroll`/`tfa-verify` Edge Fns, `App.tsx` (context menu + re-lock + cold-start gate) | Vault key = HKDF-SHA256(masterRaw, salt=user_id, info='vault:v1'); body wrapped as `vault:<n>:<ct>` before outer per-note AES-GCM. TOTP secret AES-GCM'd with `TFA_KEK` env var; cold-start TFA gate for Business+. QR rendered via `api.qrserver.com` (recommend swapping in a self-contained encoder before GA). |
