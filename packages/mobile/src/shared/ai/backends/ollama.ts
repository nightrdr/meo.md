...
export class OllamaBackend implements Generator {
  ...
  async isAvailable(): Promise<boolean> {
    try {
      // AbortSignal.timeout() may not be present on older Hermes builds;
      // manual controller for portability.
      const ctrl = new AbortController();
      const t = setTimeout(() => ctrl.abort(), 800);
      try {
        const res = await fetch(`${this.endpoint}/api/tags`, { method: 'GET', signal: ctrl.signal });
        return res.ok;
